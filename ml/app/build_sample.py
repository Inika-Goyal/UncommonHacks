"""DEPRECATED — sample/synthetic-data report builder.

This script was the original `build_report.py`. It expects the
synthetic-era artifact schema (per-exploit `geo_<exploit>.joblib`
files, `EXPLOIT_TYPES` axis, year-sliderable heatmap from a multi-year
panel) and does NOT run against the current real-data artifacts.

It is kept here for reference / regression-testing of the synthetic
pipeline only. For the production real-data report see
`ml/app/build_report.py`.

To use this you would also need to regenerate the synthetic panel:
    python -m ml.data.synthetic_sample
    python -m ml.app.build_sample
"""
...
try:
    ...
    if _VENV_PY.exists() and not _in_target_venv:
        ...
        os.execv(str(_VENV_PY), [str(_VENV_PY), "-m", "ml.app.build_sample", *sys.argv[1:]])
    print(
...
from ml.data.synthetic_sample import EXPLOIT_TYPES, PREDICTOR_COLS  # noqa: E402
...
def _require(path: Path) -> None:
    if not path.exists():
        print(
            ...
            "this is the SAMPLE/SYNTHETIC report — it needs the synthetic panel.\n"
            "see ml/app/build_report.py for the real-data report.\n"
            "to regenerate the synthetic artifacts:\n"
            "  python -m ml.data.synthetic_sample\n"
            "  python -m ml.pipelines.train_geographic   # (would need synthetic-era pipeline)\n"
