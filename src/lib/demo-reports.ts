import sheinPrediction from "@/lib/demo-shein-prediction.json";
...
// Real Shein supply-chain prediction (CHN HQ + VNM + KHM manufacturing)
// generated by running `pnpm ml:train` and the predict CLI. Keeps the
// demo path showing the same rich multi-country + drivers + gap
// visualization the live agent swarm would produce.
const SHEIN_ML_PREDICTION: MlPrediction = {
  ...(sheinPrediction as unknown as MlPrediction),
  sources: ML_SOURCES,
  adjustments: {
    severityFromMl: 3,
    severityFromAgents: 1,
    credibilityFromMl: 3,
    credibilityFromAgents: 1,
    rationale:
      "Agent signals (UFLPA watchlist hit, court cases, corroborating news) added a transparent boost on top of the ML country prediction.",
  },
};
...
export const demoReports: Report[] = [
  {
    ...
    mlPrediction: SHEIN_ML_PREDICTION,
  },
  {
